...
- [§5. 심층 인터뷰 — 모듈화 & 코드 기반 확장 로드맵](#5-심층-인터뷰--모듈화--코드-기반-확장-로드맵)
- [§6. 학습 성찰 & 회고](#6-학습-성찰--회고)
...
<a id="5-심층-인터뷰--모듈화--코드-기반-확장-로드맵"></a>

## 🎤 §5. 심층 인터뷰 — 모듈화 & 코드 기반 확장 로드맵

> 자동화를 "돌아가게 만드는 것"과 "운영 가능한 시스템으로 키우는 것"은 다른 문제다. 아래는 본 프로젝트를 재사용 가능한 모듈로 재구조화하고, 향후 코드 기반 자동화로 확장하는 로드맵이다.

### 5.1 워크플로우 모듈화 · 구조화 전략

#### ① 현재의 문제 — Monolithic Scenario

프로젝트 2의 초기 구현은 **11개 모듈을 하나의 시나리오에 직렬 배치**한 monolithic 구조였다. 이 구조는 아래 3가지 문제가 있다.

| 문제 | 증상 |
|:---|:---|
| 🔗 **강한 결합도** | RSS 로직을 수정하려면 시나리오 전체를 열어 컨텍스트 파악 필요 |
| 🔁 **재사용 불가** | 다른 프로젝트에서 "OpenAI 요약" 로직만 떼서 쓰기 어려움 |
| 🐛 **디버깅 난이도 ↑** | 실패 시 11개 모듈 중 어디서 터졌는지 로그 뒤짐 |

#### ② 개선 — 3-Layer 모듈화 아키텍처

```mermaid
flowchart TB
    subgraph L1["🎯 Layer 1: Trigger Layer (진입점)"]
        T1[Schedule<br/>매일 07:30]
        T2[Webhook<br/>수동 트리거]
        T3[Gmail<br/>특정 라벨 감지]
    end
    subgraph L2["⚙️ Layer 2: Processor Modules (재사용 가능)"]
        P1[📡 RSS Collector<br/>Sub-scenario A]
        P2[🤖 AI Summarizer<br/>Sub-scenario B]
        P3[📝 Content Formatter<br/>Sub-scenario C]
    end
    subgraph L3["📤 Layer 3: Delivery Layer (출력)"]
        D1[📧 Gmail Sender]
        D2[💬 Slack Notifier]
        D3[📊 Sheets Logger]
    end
...
#### ③ 모듈 분리 원칙 (SoC — Separation of Concerns)

| 모듈 | 책임 | 인터페이스 (입력 → 출력) |
|:---|:---|:---|
| **RSS Collector** | 다중 RSS 소스에서 최근 기사 수집 · 중복 제거 | `{sources: [], hours: 24}` → `[{title, url, description}]` |
| **AI Summarizer** | 기사 배열을 받아 3줄 요약 배열 반환 | `[article]` → `[{...article, summary}]` |
| **Content Formatter** | 요약 배열을 이메일 HTML/Slack 마크다운으로 조립 | `[summary]` → `{html, markdown, plain}` |
| **Gmail Sender** | 이메일 발송 (제목·본문·수신자만 파라미터) | `{to, subject, html}` → `{messageId}` |

#### ④ Make에서의 실제 구현 방법

Make는 **Sub-scenarios(하위 시나리오 호출)** 를 통한 모듈화를 지원한다.

```text
📁 Scenarios/
├── 🎯 [Main] Daily AI Brief          ← 오케스트레이터 (진입점)
├── 📦 [Module] RSS Collector         ← Webhook Trigger로 호출
├── 📦 [Module] AI Summarizer         ← Webhook Trigger로 호출
├── 📦 [Module] Content Formatter     ← Webhook Trigger로 호출
└── 📦 [Module] Multi-Channel Sender  ← Webhook Trigger로 호출
```

**연결 방식**: 각 모듈은 자체 Webhook URL을 갖고, 메인 시나리오는 **`HTTP - Make a Request`** 모듈로 순차 호출한다. 호출부는 `POST {webhook_url}` + JSON 페이로드로 표준화.

#### ⑤ 모듈화 이후 얻은 것

| 지표 | Before (Monolithic) | After (Modular) |
|:---|:---:|:---:|
| 시나리오 수정 시 영향 범위 | 11개 모듈 전체 | 해당 서브 시나리오만 |
| "AI 요약" 로직 재사용성 | ❌ 복붙 필요 | ✅ Webhook 호출 |
| 신규 소스 추가 소요 시간 | 30분 (시나리오 전체 재검토) | 5분 (RSS Collector에만 URL 추가) |
| 실패 지점 특정 시간 | 5~10분 | 1분 이내 (실패한 서브 시나리오만 확인) |
| 팀 협업 시 충돌 | 잦음 | 거의 없음 (모듈 오너십 분리) |

> [!TIP]
> **모듈화 판단 기준**: 하나의 모듈이 **3곳 이상의 시나리오에서 재사용**될 조짐이 보이면 즉시 분리한다. 이번 프로젝트에서 `AI Summarizer`는 이후 슬랙 메시지 요약·회의록 요약에도 쓰이므로 분리 필수.

---

### 5.2 코드 기반 자동화로의 확장 로드맵

#### ① 노코드의 한계 — 언제 코드로 넘어가야 하나?

노코드 도구는 다음 상황에서 한계에 부딪힌다.

<table>
<tr><td width="30%"><b>🚧 한계 상황</b></td><td><b>구체적 증상</b></td></tr>
<tr><td>💰 <b>실행량 폭증</b></td><td>일 1,000회 이상 실행 시 Make 유료 플랜(월 $16~)이 코드 인프라(월 $5)보다 비싸짐</td></tr>
<tr><td>🔀 <b>복잡한 조건 분기</b></td><td>Router 3단 중첩 이상이면 시각 UI가 오히려 가독성 저하</td></tr>
<tr><td>🧠 <b>커스텀 로직 필요</b></td><td>정규식·문자열 파싱·수학 계산 등이 Formatter 모듈로는 답답</td></tr>
<tr><td>🔐 <b>보안·컴플라이언스</b></td><td>민감 데이터를 외부 SaaS 경유시키기 어려운 경우 (금융/헬스케어)</td></tr>
<tr><td>🧪 <b>버전 관리·테스트</b></td><td>Git 워크플로우·유닛 테스트가 노코드 도구에서는 취약</td></tr>
</table>
...
```mermaid
flowchart LR
    S1[🟢 Stage 1<br/>순수 노코드<br/>Make/Zapier] --> S2[🟡 Stage 2<br/>하이브리드<br/>Make + 코드 스니펫]
    S2 --> S3[🔴 Stage 3<br/>코드 네이티브<br/>Python/TS + Cloud]
    style S1 fill:#C8E6C9,stroke:#388E3C,stroke-width:2px
    style S2 fill:#FFF59D,stroke:#F9A825,stroke-width:2px
...
**적합:** 월 실행 < 1,000회, 표준 SaaS 연동 중심

**한계:** 커스텀 로직·대량 처리·복잡 분기
...
**도구:** Make + **Cloudflare Workers** / AWS Lambda

**전략:** Make는 오케스트레이션, 무거운 로직은 코드 함수로 위임 (`HTTP` 모듈로 호출)

**적합:** 커스텀 파싱·계산이 필요하지만 여전히 시각적 흐름은 유지하고 싶을 때
...
**도구:** **Python (FastAPI + Celery + Redis)** or **TypeScript (n8n self-hosted)**

**전략:** 노코드 완전 이탈. GitHub Actions로 CI/CD, Docker로 배포

**적합:** 일 10,000+ 실행, 팀 5인+, 보안 요구 강

</td>
</tr>
</table>

#### ③ Stage 2 — 하이브리드 구현 예시 (본 프로젝트 확장 시)

`AI Summarizer` 모듈만 **Cloudflare Workers 함수**로 대체하는 시나리오:

```typescript
// summarize.ts (Cloudflare Workers)
export default {
      ...
      articles.map(async (article) => {
        const res = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 200,
            messages: [
              { role: 'system', content: '3줄 한국어 요약 큐레이터' },
              { role: 'user', content: `[제목] ${article.title}\n[본문] ${article.description}` },
            ],
          }),
        });
        const data = await res.json();
        return { ...article, summary: data.choices[0].message.content };
      })
    );
...
```
[HTTP - Make a Request]
  URL:    https://ai-summarizer.***마스킹***.workers.dev
  Method: POST
  Body:   { "articles": {{RSS_Collector.output}} }
```
...
| 항목 | 순수 Make | 하이브리드 |
|:---|:---:|:---:|
| OpenAI API 병렬 호출 | ❌ 순차 (Iterator) | ✅ `Promise.all` 병렬 |
| 6건 요약 소요 시간 | ~15초 | ~3초 |
| 재시도 로직 정교함 | 3회 고정 | Exponential backoff 커스텀 |
| 월 비용 (일 6건 기준) | 무료 (Ops 여유 있음) | 무료 (Workers 무료 티어) |
| 워크플로우 가독성 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ (모듈이 더 명확) |

#### ④ Stage 3 — 코드 네이티브 전환 시 아키텍처

```mermaid
flowchart TB
    CRON[⏰ APScheduler<br/>Cron Job] --> API[⚡ FastAPI<br/>Orchestrator]
    API --> Q[📨 Celery Queue<br/>Redis Backend]
    Q --> W1[👷 Worker 1<br/>RSS Fetcher]
    Q --> W2[👷 Worker 2<br/>AI Summarizer]
    Q --> W3[👷 Worker 3<br/>Email Sender]
    W1 --> DB[(🗄️ PostgreSQL<br/>Article Cache)]
    W2 --> DB
    W3 --> LOG[📊 Grafana<br/>Metrics]
    W1 -.->|실패| SEN[🛰️ Sentry<br/>Error Tracking]
    W2 -.->|실패| SEN
    W3 -.->|실패| SEN
    style API fill:#B3E5FC,stroke:#0288D1,stroke-width:2px
    style Q fill:#FFE0B2,stroke:#EF6C00
    style DB fill:#DCEDC8,stroke:#558B2F
    style SEN fill:#F8BBD0,stroke:#AD1457
```
...
| 노코드 요소 | 코드 네이티브 대체 | 왜? |
|:---|:---|:---|
| Make Schedule Trigger | **APScheduler** or GitHub Actions cron | 자유로운 cron 표현식, 코드 버전 관리 |
| Make Router | Python `if/elif` + Celery routing | 복잡 분기의 가독성 · 유닛 테스트 가능 |
| Make Filter | List comprehension / SQL WHERE | 성능 최적화 · SQL 인덱스 활용 |
| Make HTTP Module | `httpx.AsyncClient` | 비동기 병렬 호출 · 재시도 미세 제어 |
| Make Error Handler | Sentry + Celery `autoretry_for` | 스택 트레이스 · 메트릭 시각화 |
| Make Sheets Log | PostgreSQL + Grafana | 쿼리 · 시계열 분석 · 알람 |
| Make Execution History | Sentry / DataDog | 검색 · 필터 · 팀 공유 |

#### ⑤ 언제 어느 Stage로 갈지 판단 체크리스트

```markdown
☑ Stage 1 → Stage 2 전환 신호
  □ 월 Make Operations가 무료 한도의 80% 도달
  □ 하나의 모듈에서 15+ 스텝 사용 중
  □ 정규식/JSON 파싱을 3회 이상 반복 사용
  □ AI API를 병렬로 호출하고 싶다는 니즈 발생

☑ Stage 2 → Stage 3 전환 신호
  □ 팀원 3명 이상이 동시 편집 필요
  □ 유닛 테스트가 필수 (금융/헬스케어 등)
  □ 일 실행량 5,000+ 도달
  □ SLA 99.9% 이상 요구
  □ 커스텀 대시보드/알람 필요
```
...
> [!IMPORTANT]
> ### 🎯 결론 — "노코드는 도착점이 아니라 출발점"
>
> 노코드는 아이디어를 **최단 시간에 검증**하기 위한 프로토타이핑 도구다. 검증이 끝나고 실행량·복잡도·팀 규모가 임계점을 넘으면, 같은 워크플로우를 **모듈화 → 하이브리드 → 코드 네이티브**로 자연스럽게 이관하는 것이 정석이다. 이번 미션의 진짜 자산은 두 개의 자동화가 아니라, **이관 가능한 형태로 설계된 아키텍처 감각** 그 자체다.
